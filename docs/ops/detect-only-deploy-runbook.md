...
이 경로는 **`X-Admin-Key` 헤더 인증**이 걸려 있다. 폐쇄망이라도 민감정보에서
파생된 집계이므로 무인증으로 열지 않는다. `X-Admin-Key` 는 표준 Authorization
헤더가 아니므로 **Prometheus 의 `authorization:` 블록으로는 인증되지 않는다**
(그 블록은 `Authorization:` 을 보낸다 → 헤더 누락으로 422). 두 방법 중 하나를 쓴다.

- **헤더 주입 프록시** — 스크레이프 대상을 프록시로 두고 프록시가 `X-Admin-Key` 를
  붙인다. 어느 Prometheus 버전에서도 된다
- **Prometheus 의 임의 헤더 설정** — 배포된 Prometheus 버전이 스크레이프 잡에
  임의 헤더를 지원하면 그것을 쓴다. **버전을 먼저 확인할 것**
...
로그에 경고가 남는다 — 그 경고가 보이면 관측이 안 되고 있는 것이다.

즉 대시보드를 가동하려면 스크레이프 대상 하나로는 부족하고 **pushgateway 가
함께 있어야 한다.**

### 3. 탐지 전용 창에서 Grafana 를 읽는 방법
