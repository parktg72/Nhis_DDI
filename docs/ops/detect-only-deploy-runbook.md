...
수집할 것 — 요청 단위로 응답에서 뽑는다.

| 필드 | 쓰임 |
|---|---|
| `rule_level` | 규칙층이 무엇을 탐지했는지. 탐지 전용에서도 그대로 실린다 |
| `risk_reasons` | 규칙 ID(`TOP01`~`TOP10`, `SEV_*`, `GRADE_*`) |
| `ddi_alerts` | 약물쌍·심각도·출처 |
| `risk_level` | 실제 개입 등급 — 배포 전과 같아야 한다 |

집계 항목:

- **규칙별 발화 환자 수** — 10종 각각. 이것이 A5 의 본체다
- **환자 단위 발화율** — 전체 요청 중 `TOP*` 사유가 하나라도 붙은 비율
- **사유 없는 Red 건수** — `risk_level`=RED 인데 `risk_reasons` 가 비어 있는 요청. **A4 활성의 차단 항목이며, 이 값이 0 임을 확인하는 것이 해제 조건이다**
