...
## 메모리 부족으로 인한 크래시 — 코드를 먼저 의심하지 말 것

다음 형태로 죽으면 **단정 실패가 아니라 프로세스 중단**이다.

```
pyarrow.lib.ArrowMemoryError: malloc of size 268435456 failed
Fatal Python error: Aborted
```

pytest 가 요약을 못 내거나, 실행마다 죽는 지점이 달라지거나, 통과 건수가 매번 다르면 이쪽이다. DDI 매트릭스(약 29MB)를 pandas 로 펼칠 때 256MB 연속 할당이 필요한데, 이것을 매 테스트마다 새로 로드하는 파일이 있다(`test_rule_detect_only.py` 는 9회). 메모리가 빠듯하면 이 파일 하나가 회귀를 통째로 죽인다.

**가르는 방법 — 코드를 되돌리기 전에 이것부터 한다.**

```bash
git stash -u                       # 작업분을 걷어내고
python -m pytest <해당 파일> -q     # 같은 실패가 나는지 본다
git stash pop                      # 반드시 복원
```

직전 커밋에서도 같은 실패가 나면 **환경 문제이며 변경과 무관하다.** 이 절차 없이 크래시를 코드 탓으로 보면 멀쩡한 수정을 되돌리게 된다(2026-09-02 실제 발생).

메모리를 확보한 뒤 재실행하는 것이 정석이고, 확보가 어려우면 변경이 실제로 닿는 파일만 골라 돌리고 **완주하지 못한 파일을 커밋 메시지에 명시**한다. "회귀 통과" 라고 쓰지 않는다.
