# scripts/train/ 단위·통합 테스트 보강 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `BaseTrainer.save/load` SHA-256 체인, `EnsembleTrainer.save` 3파일 생성, `build_trainer` 팩토리, `TrainPipeline.run` 오류 처리 단위/통합 테스트 추가.

**Architecture:** `tests/test_train/test_pipeline.py` (신규). XGBoost/LightGBM 없이 `tests/test_train/test_train.py` 내 기존 `MockTrainer` 패턴을 재사용. `TrainPipeline.run`은 MockTrainer를 `build_trainer`에 monkey-patch해 실제 모델 없이 실행.

**Tech Stack:** pytest, pickle, hashlib, numpy, pandas, unittest.mock

---

### Task 1: `BaseTrainer.save/load` 왕복 테스트 + SHA-256 검증

**Files:**
- Create: `tests/test_train/test_pipeline.py`

- [ ] **Step 1: 파일 생성 + BaseTrainer save/load 테스트 작성**
...
```bash
python3 -m pytest tests/test_train/test_pipeline.py::TestBaseTrainerSaveLoad -v
```
Expected: `5 passed`
...
```bash
git add tests/test_train/test_pipeline.py
git commit -m "test: BaseTrainer save/load SHA-256 왕복 테스트 5건"
```

---

### Task 2: `EnsembleTrainer.save` — 3파일 + SHA-256 생성 검증

**Files:**
- Modify: `tests/test_train/test_pipeline.py`

- [ ] **Step 1: MockEnsembleTrainer + EnsembleTrainer.save 테스트 추가**
...
```bash
python3 -m pytest tests/test_train/test_pipeline.py::TestEnsembleTrainerSave -v
```
Expected: `4 passed`
...
```bash
git add tests/test_train/test_pipeline.py
git commit -m "test: EnsembleTrainer.save 3파일+SHA-256 생성 테스트 4건"
```

---

### Task 3: `build_trainer` 팩토리 테스트

**Files:**
- Modify: `tests/test_train/test_pipeline.py`

- [ ] **Step 1: build_trainer 테스트 추가**
...
```bash
python3 -m pytest tests/test_train/test_pipeline.py::TestBuildTrainer -v
```
Expected: `5 passed`
...
```bash
git add tests/test_train/test_pipeline.py
git commit -m "test: build_trainer 팩토리 테스트 5건 (xgb·lgb·ensemble·unknown·타입오류)"
```

---

### Task 4: `TrainPipeline.run` — MockTrainer로 성공/실패 통합 테스트

**Files:**
- Modify: `tests/test_train/test_pipeline.py`

- [ ] **Step 1: TrainPipeline 통합 테스트 추가**
...
class TestTrainPipelineRun:
    ...
    def feature_base(self, tmp_path, rng):
        ...
        # scaler/selector stub (identity transform)
...
```bash
python3 -m pytest tests/test_train/test_pipeline.py::TestTrainPipelineRun -v
```
Expected: `4 passed`

주의: `sklearn` 필요. 설치 안 됐으면: `pip install scikit-learn`
...
```bash
git add tests/test_train/test_pipeline.py
git commit -m "test: TrainPipeline.run 통합 테스트 4건 (성공·메타데이터·데이터없음·recall임계값)"
```
