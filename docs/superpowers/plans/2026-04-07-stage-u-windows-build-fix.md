...
## 후속 수정 (Stage U T1~T3 완료 후)

### T4: pip `~ip` 잔류 dist-info 경고 제거 (커밋 15332ca → 2741f03 → 1501d24)

**현상**: build.bat 실행 시 `WARNING: Ignoring invalid distribution ~ip` 경고가 반복 출력됨.

**원인**: `pip install --upgrade pip` 실행 시 pip가 자신을 교체하면서 `~ip-XX.dist-info` 임시 디렉터리를 `venv\Lib\site-packages\`에 남김. 이후 모든 pip 호출이 시작 시 이를 감지해 경고 출력.

**초기 수정 실패 원인**: glob 패턴을 따옴표로 감쌌기 때문 — CMD `for` 루프에서 `"path\~*"` 는 `*`가 와일드카드로 확장되지 않고 리터럴로 처리됨.

**최종 수정**:
- glob 패턴에서 따옴표 제거: `for /d %%i in (venv\Lib\site-packages\~*)`
- 정리 시점 4곳: venv 활성화 직후(이전 실행 잔류물) + pip upgrade 후 + requirements.txt 후 + pyinstaller 후
- `:cleanup_dist` 서브루틴으로 추출해 중복 제거
- 따옴표 제거가 의도된 것임을 주석으로 명시 (향후 실수 방지)
...
**Codex/opencode 검토 결과**: 두 에이전트 모두 진단·수정 방법에 동의. 서브루틴 추출 및 주석 추가 권장사항 반영.
