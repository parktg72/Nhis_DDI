...
- Latest completed work commit before this summary update: `fb64ec1 feat: HANA 세션 만료 재연결 가드 추가 (Round B)`.
- Main detailed history: `docs/reviews/agent_debug_log_20260510.md` and `docs/reviews/debug_log_20260512.md`.
...
1. Run a real HANA extraction smoke test when credentials/environment are available and watch for `-10821` recurrence.
2. If runtime logs show another stale-session code/path, create a new Claude spec before expanding reconnect handling.
3. Keep GUI helper extraction separate and small; avoid broad refactors until current reason_code/error-detail and reconnect changes have runtime validation.
