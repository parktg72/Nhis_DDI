...
REM constraints-py312.txt 가 핵심 네이티브 (numpy/sklearn/xgboost/fastapi/starlette ...) 정확 핀
REM 휠셋에 다중 버전(예: fastapi 0.135.1/0.135.2)이 있을 때 결정적 선택을 강제
...
%PYTHON_BIN% -m pip install --no-index %FIND_LINKS% %CONSTRAINT% --upgrade -r "%WIN_DIR%\requirements.txt" 2>nul ^
    || echo [경고] packages_win\requirements.txt 일부 실패 — 위 핵심 패키지는 이미 설치됨
%PYTHON_BIN% -m pip install --no-index %FIND_LINKS% %CONSTRAINT% --upgrade -r "%HANA_DIR%\requirements.txt" 2>nul ^
