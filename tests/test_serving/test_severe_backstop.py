"""결정적 severe→Y_TRIPLE 백스톱 (model-independent floor).

중증 조건(triple_whammy/10drug+고위험/고령+장기)은 학습상 Y_TRIPLE(문자안내). 모델이 하향
분류해도 serving 이 최소 Y_TRIPLE 보장(Red·Y_DDI_MAJOR 등 상위 subtype 은 유지). 단방향.
"""
...
def _predictor(model_subtype: str, model_action: str, *, severe: set, red: set = frozenset()):
...
def test_severe_escalates_low_subtype_to_ytriple():
    """모델 Y_DOUBLE(모니터링) + severe → Y_TRIPLE(문자 안내) 로 상향."""
    res = _predict(_predictor("Y_DOUBLE", "모니터링", severe={"SEV_TRIPLE_WHAMMY"}))
    assert res.risk_level == RiskLevel.YELLOW
    assert res.yellow_subtype == "Y_TRIPLE"
    assert res.action == "문자 안내"
    assert "SEV_TRIPLE_WHAMMY" in res.risk_reasons


def test_severe_escalates_no_alert_to_ytriple():
    """모델 No_Alert(None subtype) + severe → Y_TRIPLE/Yellow."""
    res = _predict(_predictor("No_Alert", "관여 안 함", severe={"SEV_10DRUG_HIGHRISK"}))
    assert res.risk_level == RiskLevel.YELLOW
    assert res.yellow_subtype == "Y_TRIPLE"
    assert res.action == "문자 안내"


def test_severe_does_not_downgrade_ddi_major():
    """모델 Y_DDI_MAJOR(약사전화, 상위) + severe → 유지(하향 안 함)."""
    res = _predict(_predictor("Y_DDI_MAJOR", "약사 전화", severe={"SEV_ELDERLY_ORGAN"}))
    assert res.yellow_subtype == "Y_DDI_MAJOR"
    assert res.action == "약사 전화"


def test_no_severe_keeps_model_subtype():
    res = _predict(_predictor("Y_DOUBLE", "모니터링", severe=set()))
    assert res.yellow_subtype == "Y_DOUBLE"
    assert res.action == "모니터링"


def test_red_takes_precedence_over_severe():
    """금기(Red) + severe → Red 유지(severe 백스톱은 Red 미관여)."""
    res = _predict(_predictor("Y_DOUBLE", "모니터링",
                              severe={"SEV_TRIPLE_WHAMMY"}, red={"RED_CONTRAINDICATED"}))
